...
const Profile = () => {
  ...
  return (
    <div
      className={clsx(
        sprinkles({ display: 'flex', align: 'center', flexDirection: 'column', gap: 'medium' }),
        styles.container,
      )}
    >
      <img src={user.profileImage} alt={`${user.name}의 프로필 사진`} className={styles.profileImage} />
      <div className={styles.info}>
        <h1 className={styles.name}>{user.name}</h1>
        <p className={styles.email}>{user.email}</p>
      </div>
      <Button className={clsx(flexCenterGapSmall)} variant="secondary" onClick={toggleTheme}>
        {isDarkMode ? <Sun size={ICON_SIZE} /> : <Moon size={ICON_SIZE} />}
        <p>{isDarkMode ? 'Light Mode' : 'Dark Mode'}</p>
      </Button>
      <Button className={flexCenterGapSmall} onClick={onPasswordChange}>
        <Key size={ICON_SIZE} />
        <p>비밀번호 변경</p>
      </Button>
      <Button className={flexCenterGapSmall} variant="danger" onClick={onLogout}>
